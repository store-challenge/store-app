...
const Stepper = props => {
  const { array, currentIndex, setCurrentIndex } = props;
  ...
  return (
    <CustomMobileStepper
      ...
      nextButton={
        <CustomIconButton onClick={handleNext} disabled={array.length === 0}>
          <Icon icon="iconamoon:arrow-right-2-duotone" color="var(--secondColor)" fontSize="20px" />
        </CustomIconButton>
      }
      backButton={
        <CustomIconButton onClick={handlePrevious} disabled={array.length === 0}>
